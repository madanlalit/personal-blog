    ...
    const executeCommand = (cmd: string) => {
        ...
        switch (commandName) {
                ...
                newOutput = (
                    <div>
                        ...
                        <ul>
                            ...
                            <li>`goto [path]` - Navigate to a page.</li>
                ...
                if (args[0] === 'posts') {
                    newOutput = (
                        <ul className="commander-list">
                            {posts.map(p => (
                                <li key={p.id}>
                                    <Link href={`/post/${p.slug}`} onClick={onClose}>{p.title}</Link>
                                </li>
                            ))}
                        </ul>
                    );
            ...
            case 'goto':
                if (argString) {
                    onClose();
                    router.push(argString.startsWith('/') ? argString : `/${argString}`);
                    return;
                } else {
                    newOutput = 'Usage: goto [path]';
                }
                break;
            case 'clear':
                setOutput([]);
        ...
        setOutput(prev => [...prev, <div key={`cmd-${prev.length}`}>&gt; {cmd}</div>, <div key={`out-${prev.length}`}>{newOutput}</div>]);
    };
