...
interface RelatedPostsProps { currentPost: Post; allPosts: Post[]; }
...
const RelatedPosts: React.FC<RelatedPostsProps> = ({ currentPost, allPosts }) => {
    const relatedPosts = allPosts
        ...
        .map(post => ({
            post,
            score: (currentPost.tags?.filter(tag => post.tags?.includes(tag)).length || 0) * 3 + (post.category === currentPost.category ? 2 : 0)
        }))
        ...
        <section className="related-posts post-frame">
            ...
            <div className="related-list">
                {relatedPosts.map((post) => (
                    <Link key={post.id} to={`/post/${post.id}`} className="related-card">
